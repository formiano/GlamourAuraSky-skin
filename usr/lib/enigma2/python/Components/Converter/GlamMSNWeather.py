...
# Astral time
		...
		if ST < 0:
			ST = ST + 24
		ST = ST * 15 # Astral time at the time of calculation in degrees
...
# Earth orbit
		try:
			T = (JDN - 2451545) / 36525
			LS = 280.4664568 + 36000.7697509 * T + 0.0003032 * T * T + 0.00000002 * T * T * T # longitude cf.
			MS = 357.5291092 + 35999.0502909 * T - 0.0001537 * T * T - 0.00000004 * T * T * T # CP anomaly
			...
			DS = (1.000001018 * (1 - ES * ES)) / (ES * math.cos((MS + CS) * DEG2RAD) + 1) # Distance to Sun in A.E.
			...
			ALFA = (7.53 * math.cos(LS * DEG2RAD) + 1.5 * math.sin(LS * DEG2RAD) - 9.87 * math.sin(2 * LS * DEG2RAD)) / 60 # Time equation
			...
			SRh = SRx = SRm = SSh = SSx = SSm = SCh = SCx = SCm ='-'
# Planet orbits
		T = (JDN - 2451545) / 36525
...
# Orbit of Mercury
		try:
			LP1 = 178.179078 + 149474.07078 * T + 0.0003011 * T * T # longitude cf. L
			wP1 = 28.753753 + 0.3702806 * T + 0.0001208 * T * T # argument of perihelion w
			WP1 = 47.145944 + 1.1852083 * T + 0.0001739 * T * T # longitude of the ascending node  W
			IP1 = 7.002881 + 0.0018608 * T - 0.0000183 * T * T # tilt to the ecliptic plane i
			...
			LP = LP1 + EL
			M0 = math.fmod(174.795 + 4.092317 * (JDN - 2451545), 360) # CP anomaly
			MP = M0 - EP1 * math.sin(M0 * DEG2RAD) # Kepler equation
			DP = (0.3870986 * (1 - EP1 * EP1)) / (EP1 * math.cos((MP + LP) * DEG2RAD) + 1) # Distance to Sun in A.E.
			# Heliocentric coordinates
			...
			P1Rh = P1Rx = P1Rm = P1Sh = P1Sx = P1Sm = P1Ch = P1Cx = P1Cm ='-'
# Orbit of Venus
		try:
			LP2 = 342.767053 + 58519.21191 * T + 0.0003097 * T * T # longitude cf. L
			wP2 = 54.384186 + 0.5081861 * T - 0.0013864 * T * T # argument of perihelion w
			WP2 = 75.779647 + 0.8998500 * T + 0.0004100 * T * T # longitude of the ascending node  W
			IP2 = 3.393631 + 0.0010058 * T - 0.0000010 * T * T # tilt to the ecliptic plane i
			EP2 = 0.00682069 - 0.00004774 * T + 0.000000091 * T * T # Orbital eccentricity e
			...
			LP = LP2 + EL + CP
			M0 = math.fmod(50.416 + 1.602136 * (JDN - 2451545), 360) # CP anomaly
			MP = M0 - EP2 * math.sin(M0 * DEG2RAD) # Kepler equation
			DP = (0.7233316 * (1 - EP2 * EP2)) / (EP2 * math.cos((MP + LP) * DEG2RAD) + 1) # Distance to Sun in A.E.
			# Heliocentric coordinates
			...
			P2Rh = P2Rx = P2Rm = P2Sh = P2Sx = P2Sm = P2Ch = P2Cx = P2Cm ='-'
# Orbit of Mars
		try:
			LP4 = 293.737334 + 19141.69551 * T + 0.0003107 * T * T # longitude cf. L
			wP4 = 285.431761 + 1.0697667 * T + 0.0001313 * T * T + 0.00000414 * T * T * T # argument of perihelion w
			WP4 = 48.786442 + 0.7709917 * T - 0.0000014 * T * T - 0.00000533 * T * T * T # longitude of the ascending node  W
			IP4 = 1.850333 - 0.0006750 * T + 0.0000126 * T * T # tilt to the ecliptic plane i
			EP4 = 0.09331290 + 0.000092064 * T - 0.000000077 * T * T # Orbital eccentricity e
			...
			LP = LP4 + EL + CP
			M0 = math.fmod(19.373 + 0.524039 * (JDN - 2451545), 360) # CP anomaly
			MP = M0 - EP4 * math.sin(M0 * DEG2RAD) # Kepler equation
			DP = (1.5236883 * (1 - EP4 * EP4)) / (EP4 * math.cos((MP + LP) * DEG2RAD) + 1) # Distance to Sun in A.E.
			# Heliocentric coordinates
...
# Orbit of Jupiter
		try:
			...
			IP5 = 1.308736 - 0.0056961 * T + 0.0000039 * T * T # tilt to the ecliptic plane i
			EP5 = 0.04833475 + 0.000164180 * T - 0.0000004676 * T * T - 0.0000000017 * T * T * T # Orbital eccentricity e
			...
			LP = LP5 + EL
			M0 = math.fmod(20.020 + 0.083056 * (JDN - 2451545), 360) # CP anomaly
			MP = M0 - EP5 * math.sin(M0 * DEG2RAD) # Kepler equation
			DP = (5.202561 * (1 - EP5 * EP5)) / (EP5 * math.cos((MP + LP) * DEG2RAD) + 1) # Distance to Sun in A.E.
			# Heliocentric coordinates
			...
			P5Rh = P5Rx = P5Rm = P5Sh = P5Sx = P5Sm = P5Ch = P5Cx = P5Cm ='-'
# Orbit of Saturn
		try:
			LP6 = 266.564377 + 1223.509884 * T + 0.0003245 * T * T - 0.0000058 * T * T * T # longitude cf. L
			wP6 = 338.307800 + 1.0852207 * T + 0.00097854 * T * T + 0.00000992 * T * T * T # argument of perihelion w
			WP6 = 112.790414 + 0.8731951 * T - 0.00015218 * T * T - 0.00000531 * T * T * T # longitude of the ascending node  W
			IP6 = 2.492519 - 0.0039189 * T - 0.00001549 * T * T + 0.00000004 * T * T * T # tilt to the ecliptic plane i
			EP6 = 0.05589232 - 0.00034550 * T - 0.000000728 * T * T + 0.00000000074 * T * T * T # Orbital eccentricity e
			...
			LP = LP6 + EL
			M0 = math.fmod(317.021 + 0.033371 * (JDN - 2451545), 360) # CP anomaly
			MP = M0 - EP6 * math.sin(M0 * DEG2RAD) # Kepler equation
			DP = (9.554747 * (1 - EP6 * EP6)) / (EP6 * math.cos((MP + LP) * DEG2RAD) + 1) # Distance to Sun in A.E.
			# Heliocentric coordinates
			...
			P6Rh = P6Rx = P6Rm = P6Sh = P6Sx = P6Sm = P6Ch = P6Cx = P6Cm ='-'
# Orbit of Uranus
		try:
			LP7 = 244.197470 + 429.863546 * T + 0.0003160 * T * T - 0.00000060 * T * T * T # longitude cf. L
			wP7 = 98.071581 + 0.9857650 * T - 0.0010745 * T * T - 0.00000061 * T * T * T # argument of perihelion w
			WP7 = 73.477111 + 0.4986678 * T + 0.0013117 * T * T # longitude of the ascending node  W
			IP7 = 0.772464 + 0.0006253 * T + 0.0000395 * T * T # tilt to the ecliptic plane i
			EP7 = 0.0463444 - 0.00002658 * T + 0.000000077 * T * T # Orbital eccentricity e
			...
			LP = LP7 + EL
			M0 = math.fmod(141.050 + 0.011698 * (JDN - 2451545), 360) # CP anomaly
			MP = M0 - EP7 * math.sin(M0 * DEG2RAD) # Kepler equation
			DP = (19.21814 * (1 - EP7 * EP7)) / (EP7 * math.cos((MP + LP) * DEG2RAD) + 1) # Distance to Sun in A.E.
			# Heliocentric coordinates
			...
			P7Rh = P7Rx = P7Rm = P7Sh = P7Sx = P7Sm = P7Ch = P7Cx = P7Cm ='-'
# Orbit of Neptune
		try:
			LP8 = 84.457994 + 219.885914 * T + 0.0003205 * T * T - 0.00000060 * T * T * T # longitude cf. L
			wP8 = 276.045975 + 0.3256394 * T + 0.00014095 * T * T + 0.000004113 * T * T * T # argument of perihelion w
			WP8 = 130.681389 + 1.0989350 * T + 0.00024987 * T * T - 0.000004718 * T * T * T # longitude of the ascending node  W
			IP8 = 1.779242 - 0.0095436 * T - 0.0000091 * T * T # tilt to the ecliptic plane i
			EP8 = 0.00899704 + 0.000006330 * T - 0.000000002 * T * T # Orbital eccentricity e
			...
			LP = LP8 + EL
			M0 = math.fmod(256.225 + 0.005965 * (JDN - 2451545), 360) # CP anomaly
			MP = M0 - EP8 * math.sin(M0 * DEG2RAD) # Kepler equation
			DP = (30.10957 * (1 - EP8 * EP8)) / (EP8 * math.cos((MP + LP) * DEG2RAD) + 1) # Distance to Sun in A.E.
			# Heliocentric coordinates
...
# Orbit of the moon
		try:
			...
			MRh = MRx = MRm = MSh = MSx = MSm = MCh = MCx = MCm ='-'
# Azimuth
# Planet orbits
		T = (JD - 2451545) / 36525
		MP1 = 102.27938 + 149472.51529 * T + 0.000007 * T * T
...
# Orbit of Neptune
	...
	#		AP8 = 30.10957 # semi-major axis of the orbit a
			...
			P8A ='-'
# Lunar phases, lunar distance, lunar azimuth
		try:
			LM = 218.3164477 + 481267.88123421 * T - 0.0015786 * T * T + T * T * T / 538841 - T * T * T * T / 65194000 # lunar longitude cf.
			FM = 93.272095 + 483202.0175233 * T - 0.0036539 * T * T - T * T * T / 3526000 + T * T * T * T / 863310000 # ср argument of latitude of the moon
			DM = 297.8501921 + 445267.114034 * T - 0.0018819 * T * T + T * T * T / 545868 - T * T * T * T / 113065000 # ср moon elongation
			...
			Mdist = int(385000.56 + ER * 1000) # lunar distance in km
			RA = math.atan2((math.sin(MLong * DEG2RAD) * math.cos(EPS * DEG2RAD) - math.tan(MLat * DEG2RAD) * math.sin(EPS * DEG2RAD)) , math.cos(MLong * DEG2RAD)) * RAD2DEG # right ascension
			if RA < 0:
				RA = RA + 2 * PI
			DEC = math.asin(math.sin(MLat * DEG2RAD) * math.cos(EPS * DEG2RAD) + math.cos(MLat * DEG2RAD) * math.sin(EPS * DEG2RAD) * math.sin(MLong * DEG2RAD)) * RAD2DEG # declension
