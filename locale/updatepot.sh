...
#
printf "Creating temporary file Glam-py.pot\n"
find $findoptions ../usr/ -name "*.py" -exec xgettext --no-wrap -L Python --from-code=UTF-8 -kpgettext:1c,2 --add-comments="TRANSLATORS:" -d Glam -s -o Glam-py.pot {} \+
$localgsed --in-place Glam-py.pot --expression=s/CHARSET/UTF-8/
printf "Creating temporary file enigma2-xml.pot\n"
which python
if [ $? -eq 0 ]; then
	find $findoptions ../usr/lib/ -name "*.xml" -exec python xml2po.py {} \+ > Glam-xml.pot
else
	find $findoptions ../usr/lib/ -name "*.xml" -exec python3 xml2po.py {} \+ > Glam-xml.pot
fi
printf "Merging pot files to create: enigma2.pot\n"
cat Glam-py.pot Glam-xml.pot | msguniq -s --no-wrap --no-location -o Glam.pot -
#printf "remove pot Creation date\n"
#$localgsed -i -e'/POT-Creation/d' Glam.pot
printf "remove temp pot files\n"
rm Glam-py.pot Glam-xml.pot
